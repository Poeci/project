...
x = np.matrix(temp)
x = x.T
count_without_space = 0
count_without_endl = 0
count_wihtout_double_endl = 0
space_every = 7
endl_every = 50
double_endl_every = 200 
jk = 0
for i in range(input_size-1,500):
	...
	#print a
	b = 0
	count_without_endl += 1
	count_without_space += 1
	...
	if (max_ind == '\n'):
		count_without_endl = 0
	if (count_without_space == 7):
		count_without_space = 0
		wiersz.write(' ')
	if (count_without_endl == 30):
		count_without_endl = 0
		wiersz.write('\n')
	if(jk%200 == 0):
		wiersz.write('\n')
